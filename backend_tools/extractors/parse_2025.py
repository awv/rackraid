# backend_tools/extractors/parse_2025.py
import re
import pdfplumber
import json
import os
...
def extract_2025(pdf_path):
    ...
    print(f"-> Parsing stable digital text layer for 2025: {pdf_path}")
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            for line in text.split('\n'):
                ...
                if any(x in line.lower() for x in ["distance :", "result :", "total time after"]):
                    continue
                ...
                # --- STAGE 13 ENGINE ---
                    ...
                    if len(bracket_split) >= 2:
                        runner_name = bracket_split[0].strip()
                        runner_club = bracket_split[1].strip()
                    else:
                        tokens = right_side.split()
                        if len(tokens) >= 2:
                            runner_name = f"{tokens[0]} {tokens[1]}"
                            runner_club = " ".join(tokens[2:]) if len(tokens) > 2 else "Independent"
                        else:
                            continue
                ...
                # --- STAGES 2 - 12 ENGINE ---
                    ...
                    profile_part = re.split(r'\s+\d+\s+\(\d+\)', right_side)[0].strip()
                    
                    bracket_split = re.split(r'\s*\(\d+\)\s*', profile_part)
                    if len(bracket_split) >= 2:
                        runner_name = bracket_split[0].strip()
                        runner_club = bracket_split[1].strip()
                    else:
